...
#The code here allows prediction to be done on website through Flask
...
def predict():
    try:
        ...
        if(int(cap.get(cv2.CAP_PROP_FRAME_COUNT))>18000):
            return render_template('index.html',prediction="Video is too length!",confidence="Please upload video not more than 10 minutes")
        videoPath=[videoPath]
        model = Model(2).cuda()
        ...
        if pred[0]==1:
            ...
            print(p)
        print(pred[1])
        return render_template("index.html",prediction=("Video is "+p),confidence=("Confidence is "+str(pred[1])))
    except:
        return render_template('index.html',prediction="Video is not received",confidence="Please upload video file!")
